...
myManageHook = composeAll
    ...
    , className =? "Main.rkt"       --> doFloatAt 0.6 0.2
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  -- Restart xmonad.
  , ((modMask, xK_q),
     spawn "dunstify recompilling && xmonad --recompile && xmonad --restart && dunstify restarted")
  ]
  ++
...
myLogHook = do
  ...
  io $ do 
    appendFile "/tmp/.xmonad-title-log" (title ++ "\n")
    appendFile "/tmp/.xmonad-workspace-log" (wsStr ++ "\n")
...
main = do
  ...
  spawn "polybar header"
  xmonad $ ewmh defaults {
        manageHook = manageDocks <+> myManageHook
      , startupHook = myStartupHook <+> docksStartupHook <+> setWMName "LG3D"
